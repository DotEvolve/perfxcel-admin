        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0 pb-6">
            ...
            <div className="flex-1 min-h-0">
              <MetricCard title="Courses & Catalog" icon={<BookOpen className="w-5 h-5" />}>
                <div className="flex flex-col h-full">
                  <div className="mb-6">
                    <p className="text-4xl font-extrabold text-indigo-600 mb-1">{metrics.courses.total}</p>
                    <p className="text-sm text-gray-500 font-medium">Total Published Courses</p>
                  </div>
                  <div className="flex-1 overflow-y-auto pr-2">
                    <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Top Categories</h4>
                    <div className="space-y-3">
                      {metrics.detailed.topCategories.map(cat => (
                        <div key={cat.name} className="flex items-center justify-between">
                          <span className="text-sm font-medium text-gray-700 truncate pr-4">{cat.name}</span>
                          <span className="text-sm font-semibold text-gray-900 bg-gray-100 px-2 py-0.5 rounded-full">{cat.count}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                {[
                  ...
                  <div key={status as string} className="bg-gray-50 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-2">
                      <Badge label={status as string} color={color as any} />
                    </div>
                    <span className="font-semibold text-gray-900 text-xl">{count as number}</span>
                  </div>
                ))}
              </div>
              <div className="flex-1 overflow-y-auto pr-2 border-t border-gray-100 pt-4">
                <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Most Demanded Courses</h4>
                <div className="space-y-4">
                  {metrics.detailed.mostDemanded.map(course => (
                    <div key={course.title} className="flex items-start justify-between group">
                      <span className="text-sm font-medium text-gray-700 truncate pr-4" title={course.title}>{course.title}</span>
          ...
          <MetricCard title="Enrollments & Certificates" icon={<GraduationCap className="w-5 h-5" />}>
            <div className="flex flex-col h-full">
              ...
              <div className="flex-1 overflow-y-auto pr-2 border-t border-gray-100 pt-4">
                <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Highest Completion Numbers</h4>
                <div className="space-y-4">
                  {metrics.detailed.topCompletions.map(course => (
                    <div key={course.title} className="flex items-center justify-between">
                      <div className="flex flex-col truncate pr-4">
                        <span className="text-sm font-medium text-gray-700 truncate" title={course.title}>{course.title}</span>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span className="text-xs font-semibold text-green-700 bg-green-50 px-2 py-1 rounded-md" title="Achieved">
                          {course.achieved} ✓
                        </span>
                        <span className="text-xs font-medium text-gray-500 bg-gray-100 px-2 py-1 rounded-md" title="In Progress">
                          {course.in_progress} ⋯
                        </span>
                      </div>
                    </div>
