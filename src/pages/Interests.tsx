...
import { ChevronUp, ChevronDown, Download, Trash2, ShieldX, ShieldOff, FileText, Plus, Send } from "lucide-react";
import Pagination from "../components/Pagination";
import ConfirmationModal from "../components/ConfirmationModal";

function ManualInterestModal({ isOpen, onClose, onSuccess, courses }: { isOpen: boolean, onClose: () => void, onSuccess: () => void, courses: any[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await createInterestManual({ course_id: courseId, name, email, phone, company, request_brochure: requestBrochure });
      onSuccess();
      onClose();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div className="relative inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
          <h3 className="text-lg font-medium leading-6 text-gray-900 mb-4">Add Interest</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">Course *</label>
              <select required value={courseId} onChange={e => { setCourseId(e.target.value); setRequestBrochure(false); }} className="w-full border rounded p-2 text-sm mt-1">
            ...
            <div>
              ...
              <input type="text" value={phone} onChange={e => setPhone(e.target.value)} className="w-full border rounded p-2 text-sm mt-1" />
            </div>
            <div>
            ...
            {showBrochureCheckbox && (
              <div className="flex items-center">
                <input type="checkbox" checked={requestBrochure} onChange={e => setRequestBrochure(e.target.checked)} className="h-4 w-4 text-indigo-600 rounded border-gray-300" />
                <label className="ml-2 block text-sm text-gray-900">Send Brochure</label>
              </div>
            )}
...
export default function Interests() {
  ...
  const [resendingBrochureIds, setResendingBrochureIds] = useState<Set<string>>(new Set());
  const [resendBrochureResults, setResendBrochureResults] = useState<Record<string, string>>({});
  ...
  const handleUpdateStatus = async (id: string, status: string) => {
    ...
    try {
      ...
      setConvertError((prev) => ({ ...prev, [id]: axiosErr.response?.data?.message ?? "Failed to update status" }));
    } finally {
      setUpdating(null);
    }
  };
  ...
  const executeConversion = async (id: string) => {
    ...
    try {
      ...
      setConvertError((prev) => ({ ...prev, [id]: err.response?.data?.message || "Failed to convert" }));
    } finally {
      setConverting(null);
    }
  };
  ...
  const SortIcon = ({ field }: { field: string }) => {
    ...
    return sortOrder === "asc" ? <ChevronUp className="w-4 h-4 inline-block ml-1" /> : <ChevronDown className="w-4 h-4 inline-block ml-1" />;
  };
    ...
    try {
      ...
      const csvContent = [
        ...
        ...res.data.map((i: any) => {
          ...
          return [date, escapeCSV(i.name), escapeCSV(i.email), escapeCSV(i.phone), escapeCSV(i.courses?.title), escapeCSV(i.company), escapeCSV(i.status)].join(",");
        }),
      ].join("\n");
  ...
  const handleResendBrochure = async (id: string) => {
    ...
    try {
      ...
      setResendBrochureResults(prev => ({ ...prev, [id]: res.regenerated ? "Link regenerated & sent" : "Link resent" }));
    } catch (err: any) {
      setResendBrochureResults(prev => ({ ...prev, [id]: "Failed to resend" }));
    } finally {
  ...
  const handleDeleteConfirm = async () => {
    ...
    finally { setConfirmDeleteIds(null); }
  };
  ...
  const handleEraseConfirm = async () => {
    ...
    try {
      ...
      loadInterests();
    } catch (err) { alert("Failed to erase data."); }
    finally { setConfirmEraseId(null); }
  };
  ...
  return (
    <div className="space-y-6">
                ...
                <thead className="bg-gray-50 select-none">
                  <tr>
                    <th className="px-6 py-3 text-left w-12">
                      <input type="checkbox" onChange={toggleSelectAll} checked={interests.length > 0 && selectedIds.size === interests.length} className="rounded border-gray-300" />
                    </th>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer hover:bg-gray-100" onClick={() => handleSort("company")}>
                  ...
                  {interests.map((interest) => (
                    <tr key={interest.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4">
                        <input type="checkbox" checked={selectedIds.has(interest.id)} onChange={() => toggleSelect(interest.id)} className="rounded border-gray-300" />
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900 flex items-center gap-1">
                          {interest.is_hard_deleted && <ShieldOff size={14} className="text-gray-400" />}
                          {interest.is_hard_deleted ? <span className="italic text-gray-500">Erased</span> : interest.name}
                          {!interest.is_hard_deleted && interest.brochure_token && <span title="Brochure Requested"><FileText size={14} className="text-indigo-500 ml-1" /></span>}
                        </div>
                        ...
                        <div className="flex flex-col items-end gap-2">
                          <div className="flex items-center gap-2">
                            <select
                              ...
                              onChange={(e) => {
                                if (e.target.value === "enrolled") handleConvertToEnrollment(interest.id);
                                else handleUpdateStatus(interest.id, e.target.value);
                              }}
                              disabled={updating === interest.id || converting === interest.id || interest.status === "enrolled" || interest.is_hard_deleted}
                            ...
                            >
                              ...
                              <option value="enrolled">Enrolled</option><option value="rejected">Rejected</option>
                            </select>
                            <button
                              onClick={() => handleConvertToEnrollment(interest.id)}
                              disabled={converting === interest.id || interest.status === "enrolled" || interest.is_hard_deleted}
                            ...
                            >
                              {converting === interest.id ? "Converting..." : interest.status === "enrolled" ? "Converted" : "Convert to Enrollment"}
                            </button>
                          </div>
                          
                          <div className="flex items-center gap-3">
                            {resendBrochureResults[interest.id] && <span className="text-xs text-green-600">{resendBrochureResults[interest.id]}</span>}
                            {!interest.is_hard_deleted && interest.brochure_token && (
                              <button onClick={() => handleResendBrochure(interest.id)} disabled={resendingBrochureIds.has(interest.id)} className="text-indigo-600 hover:text-indigo-900 flex items-center gap-1" title="Resend Brochure">
                                <Send size={16} /> <span className="text-xs">Resend</span>
                              </button>
                            )}
                            {!interest.is_hard_deleted && (
                              <button onClick={() => setConfirmEraseId(interest.id)} className="text-orange-600 hover:text-orange-900 flex items-center gap-1" title="Erase PII (GDPR)">
                                <ShieldX size={16} /> <span className="text-xs">Erase</span>
                              </button>
                            )}
                            <button onClick={() => setConfirmDeleteIds([interest.id])} className="text-red-600 hover:text-red-900" title="Delete">
                              <Trash2 size={16} />
                            </button>
                          </div>
                          
                          {convertError[interest.id] && <div className="text-red-500 text-xs">{convertError[interest.id]}</div>}
                        </div>
                      </td>
                    </tr>
                  ))}
                  {interests.length === 0 && <tr><td colSpan={7} className="px-6 py-8 text-center text-gray-500">No registered interests match your filters.</td></tr>}
                </tbody>
              </table>
            </div>
      ...
      <ManualInterestModal isOpen={showAddModal} onClose={() => setShowAddModal(false)} onSuccess={() => { loadInterests(); }} courses={courses} />
